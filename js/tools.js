...
function renderToolCard(tool) {
  ...
  console.log(`Rendering tool: ${tool.title}, solidColor: ${solidColor}`); // Debug log
  return `
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set solid color as CSS custom property for hover effect
  document.querySelectorAll(".tool-icon").forEach((icon) => {
    const solidColor = icon.getAttribute("data-solid-color");
    console.log("Setting solid color for icon:", solidColor); // Debug log
    if (solidColor) {
      icon.style.setProperty("--solid-color", solidColor);
    } else {
      console.warn("No solid color found for icon:", icon);
    }
  });
});